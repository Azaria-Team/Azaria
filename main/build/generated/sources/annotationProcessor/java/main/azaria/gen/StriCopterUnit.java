...
public class StriCopterUnit extends Unit implements Shieldc, Teamc, Entityc, Physicsc, Posc, Itemsc, StriCopterc, Weaponsc, Hitboxc, Syncc, Unitc, Drawc, Statusc, Healthc, Velc, Builderc, Rotc, Minerc {
    ...
    public float bladeMoveSpeedScl = 1.0F;
    ...
    public void writeSync(Writes write) {
        ...
        for(int INDEX = 0; INDEX < this.blades.length; INDEX ++) {
        }
    ...
    public void statusSpeed(float speed) {
        applyDynamicStatus().speedMultiplier = speed / (type.speed * 60.0F / tilesize);
    }
    ...
    public void setProp(LAccess prop, double value) {
        ...
        case shield -> shield = Math.max((float)value, 0.0F);
        case x -> {
        ...
        case velocityX -> vel.x = (float)(value * tilesize / 60.0);
        case velocityY -> vel.y = (float)(value * tilesize / 60.0);
        ...
        case speed -> statusSpeed(Mathf.clamp((float)value, 0.0F, 1000.0F));
        case armor -> statusArmor(Math.max((float)value, 0.0F));
        }
    }
    ...
    public void updateDrowning() {
        ...
        if (floor != null && floor.isLiquid && floor.drownTime > 0 && canDrown()) {
            ...
            drownTime += Time.delta / (hitSize / 8.0F * type.drownTimeMultiplier * floor.drownTime);
    ...
    public void read(Reads read) {
        ...
        this.blades = new azaria.world.draw.Blade.BladeMount[blades_LENGTH];
    ...
    public void update() {
        ...
        stricopter: {
            ...
            if (dead || health() <= 0) {
                ...
                bladeMoveSpeedScl = Mathf.lerpDelta(bladeMoveSpeedScl, 0.0F, type.bladeDeathMoveSlowdown);
            } else {
                bladeMoveSpeedScl = Mathf.lerpDelta(bladeMoveSpeedScl, 1.0F, type.bladeDeathMoveSlowdown);
            }
        ...
        unit: {
            ...
            if (isFlying() != wasFlying) {
                if (wasFlying) {
                    if (tile != null) {
                        Fx.unitLand.at(x, y, floor.isLiquid ? 1.0F : 0.5F, tile.floor().mapColor);
                    }
                }
                wasFlying = isFlying();
            }
    ...
    public void statusDrag(float drag) {
        applyDynamicStatus().dragMultiplier = type.drag == 0.0F ? 0.0F : drag / type.drag;
    }
    ...
    public double sense(LAccess sensor) {
        ...
        case velocityY ->vel.y * 60.0F / tilesize;
        ...
        case armor ->armorOverride >= 0.0F ? armorOverride : armor;
        case flag ->flag;
        case speed ->type.speed * 60.0F / tilesize * speedMultiplier;
            ...
            if (!(tile.build instanceof ConstructBuild cb)) {
                if (!current.initialized && !current.breaking && Build.validPlaceIgnoreUnits(current.block, team, current.x, current.y, current.rotation, true, true)) {
                    if (Build.checkNoUnitOverlap(current.block, current.x, current.y)) {
                        ...
                        if (hasAll) {
                            Call.beginPlace(this, current.block, team, current.x, current.y, current.rotation, current.block.instantBuild ? current.config : null);
    ...
    public static StriCopterUnit create() {
        return new StriCopterUnit();
    }

    @Override
    public int classId() {
        return AZEntityMapping.classId(azaria.gen.StriCopterUnit.class);
    }
